...
private final class TuiChild: @unchecked Sendable {
    ...
    func waitExit(timeoutSeconds: Double) -> Int32? {
...
@Test func test_tui_live_pty_render() async throws {
    ...
    let exitStatus = child.waitExit(timeoutSeconds: 3.0)
    // Drain anything still in the master's buffer (atexit handler
    // writes its cleanup sequence; we want to assert on it).
